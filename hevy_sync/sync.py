"""This module syncs measurement data from Hevy to Garmin."""
import argparse
import time
import sys
import os
...
from hevy_sync.garmin import GarminConnect
from hevy_sync.fit import FitEncoderWeight, FitEncoderBloodPressure
...
def get_args():
    ...
    parser = argparse.ArgumentParser(
        description=(
            "A tool for synchronisation of Hevy "
...
def prepare_syncdata(height, groups):
    ...
    for group in groups:
        ...
        if "weight" not in group_data and not (
            ...
            logging.info(
                "%s This Hevy metric contains no %s.  Not syncing...", dt, collected_metrics
            )
...
def sync():
    ...
    height = hevy.get_height()
    groups = hevy.get_measurements(startdate=startdate, enddate=enddate)
...
def main():
    ...
    logging.debug("hevy-sync script version %s", version("hevy-sync"))
